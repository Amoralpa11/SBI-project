...
def get_surface(model, probe_radius,  PDB_TO_XYZR=None, MSMS="msms"):
    ...
    MSMS = MSMS + " -probe_radius " + str(probe_radius) + " -if %s -of %s > " + tempfile.mktemp()
...
class ResidueDepth(AbstractPropertyMap):
    ...
    def __init__(self, model, probe_radius, pdb_file=None):
        ...
        surface = get_surface(model, probe_radius)
